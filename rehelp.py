...
class HelpMod(loader.Module):
    ...
    strings = {"name": "Help",
               ...
               "single_mod_header": "<b>🦊 Инфо о модуле</b> <u>{}</u>:",
               "single_cmd": "\n🧊 {}\n",
    ...
    async def helpcmd(self, message):
        ...
        if args:
            ...
            reply = self.strings("single_mod_header", message).format(utils.escape_html(name), utils.escape_html(
                (self.db.get(main.__name__, "command_prefix", False) or ".")[0]))
            if module.__doc__:
                reply += "<i>\nℹ️ " + "\n".join(
                    "  " + t for t in utils.escape_html(inspect.getdoc(module)).split("\n")) + "\n</i>"
            commands = {name: func for name, func in module.commands.items() if
                        await self.allmodules.check_security(message, func)}
            ...
            for mod in self.allmodules.modules:
                if len(mod.commands) != 0:
                    ...
                    first = True
                    commands = [name for name, func in mod.commands.items() if
                                await self.allmodules.check_security(message, func) or force]
                    for cmd in commands:
                        if first:
